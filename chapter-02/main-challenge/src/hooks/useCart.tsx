...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const cartVerify = cart.findIndex(c => c.id === productId);
      const response = await api.get(`/stock/${productId}`);
      const stock = response.data.amount;
      let tmpCart = [...cart];
      if (cartVerify !== -1) {
        if (cart[cartVerify].amount >= stock) {
          toast.error('Quantidade solicitada fora de estoque');
          return;
        }
        tmpCart[cartVerify].amount += 1;
      } else {
  ...
  const removeProduct = (productId: number) => {
    try {
      const productVerify = cart.find(c => c.id === productId);
      if (!productVerify) {
  ...
  const updateProductAmount = async ({
    ...
    try {
      const productVerify = cart.findIndex(c => c.id === productId);
      if (productVerify === -1) {
        throw new Error();
      }
      if (amount <= 0) return;
      ...
      const tmpCart = [...cart];
      tmpCart[productVerify].amount = amount;
      localStorage.setItem("@RocketShoes:cart", JSON.stringify(tmpCart));
      setCart(tmpCart);
    } catch {
